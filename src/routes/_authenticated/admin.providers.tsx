...
import { ShieldCheck, Search, Briefcase } from "lucide-react";
import { listAdminProviders } from "@/lib/admin.functions";
import {
  PageSkeleton,
  EmptyState,
  ErrorState,
                ...
                {filtered.map((p) => (
                  <tr key={p.id} className="border-t border-border hover:bg-secondary/40">
                    <td className="px-3 py-2">
                      ...
                      {p.headline && <p className="text-[11px] text-muted-foreground">{p.headline}</p>}
                    </td>
                    <td className="px-3 py-2">
                    ...
                    <td className="px-3 py-2 text-right tabular-nums">${(p.hourly_rate_cents / 100).toFixed(0)}/hr</td>
                    <td className="px-3 py-2 text-right tabular-nums">{p.years_experience ?? "—"}</td>
