...
function IdentityFlow() {
  ...
  const hasDoc = (k: string) =>
    documents.some((d) => d.kind === k && (d.status === "uploaded" || d.status === "accepted"));
  ...
  return (
    <div className="mx-auto max-w-2xl px-4 py-6">
        ...
        <div className="space-y-4">
          ...
          <ul className="space-y-2 text-sm">
            <li className="flex gap-2">
              <Check className="size-5 text-primary" /> Your Social Security number is never stored
              on our servers.
            </li>
            <li className="flex gap-2">
              <Check className="size-5 text-primary" /> Your ID photos are encrypted and only
              reviewers see them.
            </li>
            <li className="flex gap-2">
              <Check className="size-5 text-primary" /> You can pause and resume any time — your
      ...
      {step === "driver" && (
        <StepShell title="Will you drive for CompanionCare?" onBack={() => go("addresses")}>
          ...
          <div className="grid gap-2">
            <Button
              variant={isDriver === true ? "default" : "outline"}
              onClick={() => setIsDriver(true)}
            >
              Yes, I'll drive
            </Button>
            <Button
              variant={isDriver === false ? "default" : "outline"}
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div>
                  ...
                  <Input
                    type="date"
                    value={dlExpires}
                    onChange={(e) => setDlExpires(e.target.value)}
                  />
                </div>
              </div>
            </div>
          ...
          <Button
            className="w-full"
            disabled={
              busy || isDriver === null || (isDriver && (!dlNumber || !dlState || !dlExpires))
            }
            onClick={async () => {
              setBusy(true);
              try {
                if (isDriver) {
                  await saveDLFn({
                    data: {
                      ...
                      drivers_license_expires_on: dlExpires,
                    },
                  });
                } else {
                  await saveDLFn({
      ...
      {step === "consents" && (
        <StepShell
          title={`Disclosures (${Math.min(consentIdx + 1, remainingConsents.length || 1)} of ${remainingConsents.length || 1})`}
          onBack={() => go("driver")}
        >
          {remainingConsents.length === 0 ? (
            <>
              <p className="text-sm">You've already signed every required disclosure.</p>
              <Button className="w-full" onClick={() => go("id")}>
      ...
      {step === "id" && (
        <StepShell title="Photo of your government ID" onBack={() => go("consents")}>
          <p className="mb-4 text-xs text-muted-foreground">
            This copy goes to our staff for manual document review. After onboarding, you'll do a
            separate automated ID check (a quick liveness scan) before your background check can
            clear — that one's a different, faster step, not a repeat of this.
          </p>
          {!hasDoc("id_front") ? (
            <IDCaptureStep
              kind="id_front"
            ...
            <>
              <p className="text-sm text-emerald-700 flex items-center gap-2">
                <CheckCircle2 className="size-4" /> ID captured.
              </p>
              <Button className="w-full" onClick={() => go("selfie")}>
                Continue <ArrowRight className="ml-2 size-4" />
              </Button>
            </>
          )}
      ...
      {step === "selfie" && (
        <StepShell title="Selfie" onBack={() => go("id")}>
          {!hasDoc("selfie_liveness") ? (
            ...
            <>
              <p className="text-sm text-emerald-700 flex items-center gap-2">
                <CheckCircle2 className="size-4" /> Selfie captured.
              </p>
              <Button className="w-full" onClick={() => go("ssn")}>
                Continue <ArrowRight className="ml-2 size-4" />
              </Button>
            </>
          )}
        ...
        <StepShell title="Review & submit" onBack={() => go("ssn")}>
          <ul className="space-y-2 text-sm">
            <ReviewItem
              label="Legal name"
              ok={!!(first && last)}
              value={`${first} ${middle ? middle + " " : ""}${last}`}
            />
            ...
            <ReviewItem
              ...
              value={
                current
                  ? `${current.line1}, ${current.city}, ${current.state} ${current.postal}`
                  : ""
              }
            />
            ...
            <ReviewItem
              label="SSN"
              ok={!!identity.ssn_last4}
              value={identity.ssn_last4 ? `•••-••-${identity.ssn_last4}` : ""}
            />
            <ReviewItem
              label="ID photos"
              ok={hasDoc("id_front") && (hasDoc("id_back") || hasDoc("passport"))}
              value=""
            />
            <ReviewItem label="Selfie" ok={hasDoc("selfie_liveness")} value="" />
            <ReviewItem
              label="Disclosures signed"
              ok={remainingConsents.length === 0}
              value={`${consents.length} signed`}
            />
          </ul>
          <Button
            className="w-full"
          ...
          <Link to="/provider" className="block text-center text-sm text-muted-foreground">
...
function ReviewItem({ label, ok, value }: { label: string; ok: boolean; value: string }) {
  return (
    <li className="flex items-start justify-between gap-3 rounded-md border p-3">
      ...
      <span
        className={`mt-1 inline-flex size-6 shrink-0 items-center justify-center rounded-full ${ok ? "bg-primary/10 text-primary" : "bg-destructive/10 text-destructive"}`}
      >
