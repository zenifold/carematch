...
import { listLedger, markPayoutPaid, createRefund, type LedgerRow } from "@/lib/admin-finance.functions";
...
function toCsv(rows: LedgerRow[]) {
  ...
  const body = rows
    .map((r) =>
      [r.id, r.entry_type, r.status, (r.amount_cents / 100).toFixed(2), r.currency, r.senior_name ?? "", r.provider_name ?? "", (r.memo ?? "").replace(/"/g, '""'), r.created_at, r.posted_at ?? ""]
        .map((v) => `"${v}"`)
        .join(","),
    )
    .join("\n");
    ...
    <div className="space-y-5">
      ...
      <section className="rounded-2xl border border-border bg-card">
        <div className="flex flex-wrap items-center gap-2 border-b border-border p-3">
          <select value={entryType} onChange={(e) => setEntryType(e.target.value)} className="rounded-lg border border-input bg-background px-2 py-1.5 text-sm">
          ...
          <select value={status} onChange={(e) => setStatus(e.target.value)} className="rounded-lg border border-input bg-background px-2 py-1.5 text-sm">
        ...
        {q.isPending ? (
          <div className="p-4"><PageSkeleton title="ledger" cards={4} /></div>
        ) : q.isError ? (
          <ErrorState error={q.error} onRetry={() => q.refetch()} />
        ) : (q.data ?? []).length === 0 ? (
          <div className="p-6"><EmptyState icon={<DollarSign className="size-6" />} title="No entries" description="Nothing matches these filters." /></div>
        ) : (
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody>
                {(q.data ?? []).map((r) => <LedgerRowView key={r.id} row={r} />)}
              </tbody>
            </table>
          </div>
        )}
...
function LedgerRowView({ row }: { row: LedgerRow }) {
  ...
  const refundM = useMutation({
    mutationFn: (amount: number) => refundFn({ data: { charge_id: row.id, amount_cents: amount } }),
  ...
  return (
    <tr className="border-t border-border">
      <td className="px-3 py-2">
        <span className={`rounded px-1.5 py-0.5 text-[10px] font-semibold uppercase ${entryTone[row.entry_type]}`}>
      ...
      <td className="px-3 py-2 text-right">
        {row.entry_type === "provider_payout" && row.status === "pending" && (
          <button onClick={() => payM.mutate()} className="rounded border border-input bg-card px-2 py-1 text-xs hover:bg-secondary">
            Mark paid
          </button>
        )}
        {row.entry_type === "charge" && row.status === "posted" && (
          <button
            onClick={() => {
              const raw = window.prompt(`Refund amount in cents (max ${row.amount_cents}):`, String(row.amount_cents));
              const n = raw ? parseInt(raw, 10) : NaN;
              if (Number.isFinite(n) && n > 0) refundM.mutate(n);
            }}
