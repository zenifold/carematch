...
function ProviderMessages() {
  ...
  if (convosQ.isPending) return <PageSkeleton title="messages" cards={3} />;
  if (convosQ.isError) {
    return <ErrorState title="We couldn't load your messages" error={convosQ.error} onRetry={() => convosQ.refetch()} />;
  }
  ...
  const activeName =
    profilesQ.data?.find((p) => p.id === active?.other_user_id)?.full_name ?? "Family";
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="rounded-2xl border border-amber-200 bg-amber-50 p-4 text-sm text-amber-900">
        ...
        Keep conversations on-platform — off-platform contact violates the provider agreement.
      </div>
      ...
      {rows.length === 0 ? (
        <EmptyState
          ...
          description="Once a family reaches out about a booking, the thread appears here."
        />
      ) : (
        <div className="surface-card grid overflow-hidden lg:grid-cols-[280px_1fr]">
          ...
          <div className="flex min-h-[420px] flex-col">
            <div className="border-b border-border p-4">
              ...
              <p className="text-xs text-muted-foreground">On-platform messaging · encrypted at rest</p>
            </div>
            <div className="flex-1 overflow-y-auto p-5">
              {msgsQ.isPending ? (
                <PageSkeleton cards={2} />
              ) : msgsQ.isError ? (
                <ErrorState title="We couldn't load this thread" error={msgsQ.error} onRetry={() => msgsQ.refetch()} />
              ) : (msgsQ.data ?? []).length === 0 ? (
            ...
            <div className="flex flex-wrap gap-2 border-t border-border px-3 pt-3">
              {["On my way", "Running 5 min late", "Arrived", "All done — check-out"].map(
                (q) => (
                  <button
                    key={q}
                    type="button"
