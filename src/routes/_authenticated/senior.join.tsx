...
export const Route = createFileRoute("/_authenticated/senior/join")({
  component: SeniorJoin,
...
function SeniorJoin() {
  ...
  const lookupFn = useServerFn(lookupSeniorInvite);
  const redeemFn = useServerFn(redeemSeniorInvite);

  const previewQ = useQuery({
    queryKey: ["senior-invite-preview", previewCode],
    enabled: !!previewCode,
  ...
  const redeemM = useMutation({
    ...
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ["senior", "family-links"] });
      toast.success("You're linked. Your family can now help coordinate care.");
      navigate({ to: "/senior" });
    },
    onError: (err) =>
    ...
    <div className="mx-auto max-w-xl space-y-6">
      ...
      {previewQ.data && (
        <div className="surface-card space-y-4 p-6">
            ...
            <div>
              ...
              {previewQ.data.relationship && (
                <p className="text-sm text-muted-foreground">
                  Your {previewQ.data.relationship}
                </p>
              )}
            </div>
          </div>
          <p className="text-sm text-muted-foreground">
            They'll have{" "}
            <span className="font-semibold text-foreground">{previewQ.data.permission}</span>{" "}
            access · expires{" "}
      ...
      <p className="text-center text-xs text-muted-foreground">
        <Link to="/senior" className="underline underline-offset-2">
          Back to my home
        </Link>
      </p>
    </div>
